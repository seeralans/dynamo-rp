...
def get_bounding_rp_from_chain(chain, model_params):
  ...
  l_base = pm.rp_modules_df.query(f"L == '{l_mod}' and C == '{c_mod}' and R == '{r_mod}'")
  l_mod, c_mod, r_mod = chain[-2], chain[-1], chain[-1].split("_")[-1]
  r_base = pm.rp_modules_df.query(f"L == '{l_mod}' and C == '{c_mod}'")
  r_base = pm.rp_modules_df.query(f"L == '{l_mod}' and C == '{c_mod}' and R == '{r_mod}'")
...
def get_general_module_from_rp(rp, model_params):
  ...
  for p_vec_params, ref_frame in zip(module_params['p_vec_params'],
                                     module_params['next_ref_frames']):
